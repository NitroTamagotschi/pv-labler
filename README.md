...
3. Labels direkt auf der Bildkarte setzen (`Good` schließt Defekte aus und umgekehrt). Änderungen werden erst per `Save` gespeichert (Button oben rechts, zeigt die Anzahl geänderter Bilder) — alternativ über die Tastenkombination `Strg+S` (wirkt wie ein Klick auf `Save`, auch mit offenem Pop-up); bei ungespeicherten Änderungen fragt der Browser beim Verlassen der Seite nach.
4. Der letzte Tab `All` zeigt alle Bilder der gewählten Modalität unabhängig vom Labelstatus. Die Galerie lädt Karten in Schüben (zunächst 100; beim Scrollen ans Ende kommen automatisch weitere dazu), damit auch sehr große Bildmengen flüssig bedienbar bleiben.
